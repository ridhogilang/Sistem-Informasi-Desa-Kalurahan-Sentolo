        ...
        <section class="content-wrapper my-5">
            <article class="card content">
                ...
                <div class="main-content print space-y-4">
                    ...
                    <div class="flex flex-wrap gap-2 text-sm">
                        ...
                        <div class="inline-flex items-center">
                            ...
                            <span>Dibaca {{ $artikel->views_count }} kali</span>
                        </div>
                    </div>
                    @if ($artikel->gambar)
                        <a href="{{ Storage::url($artikel->gambar) }}" class="block" data-fancybox="images">
                            <figure>
                                <img src="{{ Storage::url($artikel->gambar) }}" alt="" class="w-full">
                            </figure>
                        </a>
                    @endif
                    @if ($artikel->artikel)
                        {!! $artikel->artikel !!}
                    @else
                        <iframe src="{{ $artikel->link }}" @if ($artikel->kategori_link == 1) width="784" height="410" @elseif ($artikel->kategori_link == 2)  width="784" height="1180" @endif></iframe>
                    @endif
                </div>
                <div id="print">
                    <style media="print">
                ...
                <div class="flex space-x-2">
                    <a href="http://www.facebook.com/sharer.php?u={{ config('app.url') }}/berita/{{ date('Y', strtotime($artikel->tanggal)) }}/{{ date('m', strtotime($artikel->tanggal)) }}/{{ date('d', strtotime($artikel->tanggal)) }}/{{ $artikel->slug }}"
                    ...
                    <a href="https://twitter.com/share?url={{ config('app.url') }}/artikel/{{ date('Y', strtotime($artikel->tanggal)) }}/{{ date('m', strtotime($artikel->tanggal)) }}/{{ date('d', strtotime($artikel->tanggal)) }}/{{ $artikel->slug }}"
                        target="_blank" rel="noopener"
                    ...
                    <a href="https://telegram.me/share/url?url={{ config('app.url') }}/artikel/{{ date('Y', strtotime($artikel->tanggal)) }}/{{ date('m', strtotime($artikel->tanggal)) }}/{{ date('d', strtotime($artikel->tanggal)) }}/{{ $artikel->slug }}"
                        target="_blank" rel="noopener"
                    ...
                    <a href="https://api.whatsapp.com/send?text={{ config('app.url') }}/artikel/{{ date('Y', strtotime($artikel->tanggal)) }}/{{ date('m', strtotime($artikel->tanggal)) }}/{{ date('d', strtotime($artikel->tanggal)) }}/{{ $artikel->slug }}"
                        target="_blank" rel="noopener"
                ...
                <div class="py-3 divide-y" id="comment">
                    @foreach ($komentar as $item)
                        <div class="flex space-x-3 py-3">
                            <img loading="lazy" src="{{ asset('home/img/user.png') }}" alt="user"
                ...
                <form action="/komentar" method="POST" class="space-y-3">
                    ...
                    <div class="flex flex-col space-y-1">
                        <input type="text" class="form-input" id="berita-id" name="berita_id"
                            value="{{ $artikel->id }}" style="display: none">
                        <input type="text" class="form-input" id="kategoriberita_id" name="kategoriberita_id"
                            value="{{ $artikel->kategoriberita_id }}" style="display: none">
                    ...
                    <button type="submit" class="button button-secondary" data-mdb-ripple="true"
                        data-mdb-ripple-color="light">Kirim Komentar <span
                            class="fa-regular fa-paper-plane ml-2"></span></button>
                </form>
            </article>
