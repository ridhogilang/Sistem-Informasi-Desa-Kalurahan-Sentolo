            ...
            <ul class="w-full space-x-3" id="button-list">
                <li class="inline-block">
                    <button id="semua-berita-link" class="button button-secondary px-5 button-s-menu"
                        ...
                        <i class="fa-regular fa-images mr-2"></i></i>Galeri
                    </button>
                </li>
            </ul>
                ...
                @if ($berita->count())
                    @foreach ($berita as $item)
                        <div class="card overflow-hidden p-4 article-wrapper">
                            <a href="/galeri/{{ date('Y', strtotime($item->created_at)) }}/{{ date('m', strtotime($item->created_at)) }}/{{ date('d', strtotime($item->created_at)) }}/{{ $item->nama }}"
                            ...
                            <div class="article-caption space-y-3 pt-3">
                                <a href="/galeri/{{ date('Y', strtotime($item->created_at)) }}/{{ date('m', strtotime($item->created_at)) }}/{{ date('d', strtotime($item->created_at)) }}/{{ $item->nama }}"
                                    class="text-heading lg:text-lg text-link block">{{ $item->nama }}</a>
                            </div>
                        </div>
                    @endforeach
